class BookmarksController < ApplicationController
  ...
  def current_user_must_be_bookmark_username
    bookmark = Bookmark.find(params[:id])

    unless current_user == bookmark.username
  ...
  def index
    @q = current_user.bookmarks.ransack(params[:q])
      @bookmarks = @q.result(:distinct => true).includes(:dish, :username, :venue).page(params[:page]).per(10)
  ...
  def create
    ...
    @bookmark.username_id = params[:username_id]
